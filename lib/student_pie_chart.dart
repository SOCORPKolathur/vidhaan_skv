...
class _StudentPieChartState extends State<StudentPieChart> {
  ...
  getvalues() async {
    ...
    var studentsDocs = await FirebaseFirestore.instance.collection("Students").get();
   
    int presentCount = 0;
    int absentCount = 0;
    setState(() {
      totalattdence = totaldays * studentsDocs.docs.length;
      holidaysenrty = holidayscount * studentsDocs.docs.length;
    });

    for(int z = 0; z < studentsDocs.docs.length; z++){
      var attendances = await FirebaseFirestore.instance.collection("Students").doc(studentsDocs.docs[z].id).collection('Attendance').where("Attendance",isEqualTo: "Present").get();
     setState(() {
       presnet = presnet + attendances.docs.length;
     });
    }
    setState(() {
...
//     totalStudentsCount = studentsDoc.docs.length;
//
//     DateTime startdate  =  DateFormat("dd / MM / yyyy").parse("01 / 06 / ${DateTime.now().year.toString()}");
//     DateTime endtdate  =  DateTime.now();
//     setState(() {
...
//     }
//
//     holidaysenrty = holidayscount * totalStudentsCount;
//
//     print("object");
...
//             // value: (presnet/totalattdence *100),
//             // title: '${(presnet/totalattdence *100).toStringAsFixed(2)}%',
//             value:  (presnet/totalattdence)*100,//(presnet/totalattdence *100),
//             title: '${((presnet/totalattdence)*100).toStringAsFixed(2)}%', //'${(presnet/totalattdence *100).toStringAsFixed(2)}%',
//             radius: radius,
//             titleStyle:  GoogleFonts.poppins(
//               fontSize: fontSize,
//               fontWeight: FontWeight.w700,
...
//         //   return PieChartSectionData(
//         //     color: Colors.red,
//         //     value:  (absent/totalattdence)*100,//(presnet/totalattdence *100),
//         //     title: '${((absent/totalattdence)*100).toStringAsFixed(2)}%', //'${(presnet/totalattdence *100).toStringAsFixed(2)}%',
//         //     // value: (absent/totalattdence *100),
//         //     // title: '${(absent/totalattdence *100).toStringAsFixed(2)}%',
//         //     radius: radius,
//         //     titleStyle:  GoogleFonts.poppins(
