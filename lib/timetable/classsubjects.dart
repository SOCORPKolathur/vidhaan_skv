...
class ClassSubjects extends StatefulWidget {
  const ClassSubjects({Key? key}) : super(key: key);
  ...
  addsubject(subname) async {
    ...
    for (int i = 0; i < document.docs.length; i++) {
      FirebaseFirestore.instance.collection("ClassMaster").doc(classid)
          .collection("Sections").doc("${_typeAheadControllerclass.text}${document.docs[i]["name"]}").collection("Subjects").doc()
          .set({
        "name": subname,
        "staffid": "",
        "staffname": "",
      ...
      builder: (BuildContext context) {
        ...
        return StatefulBuilder(
            builder: (context,setState) {
              ...
              var document = await FirebaseFirestore.instance.collection("SectionMaster").get();
              for (int i = 0; i < document.docs.length; i++) {
                FirebaseFirestore.instance.collection("ClassMaster").doc(
                    classid).collection("Sections").doc("${_typeAheadControllerclass.text}${document.docs[i]["name"]}").collection("Subjects")
                    .doc(id)
                    .delete();
              }Navigator.of(context).pop();
                    },
