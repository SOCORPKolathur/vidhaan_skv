...
class SectionMaster extends StatefulWidget {
  const SectionMaster({Key? key}) : super(key: key);

  @override
  State<SectionMaster> createState() => _SectionMasterState();
}

class _SectionMasterState extends State<SectionMaster> {
  ...
  getorderno() async {
    var document = await  FirebaseFirestore.instance.collection("SectionMaster").get();
    setState(() {
  ...
  addclass() async {
    ...
    for(int i=0;i<document.docs.length;i++) {
      FirebaseFirestore.instance.collection("Attendance").doc("${document.docs[i]["name"]}${name.text}").set({
        "name": "${document.docs[i]["name"]}${name.text}",
      });
    }
  }
  Successdialog(){
    ...
    return AwesomeDialog(
      ...
      desc: 'Section - ${name.text} is been added',
      ...
      children: [
        Padding(
          ...
          child: Container(child: Padding(
            ...
            child: Text("Section Master",style: GoogleFonts.poppins(fontSize: width/75.888888889,fontWeight: FontWeight.bold),),
          ),
            //color: Colors.white,
            width: width/1.050,
            height: height/8.21,
